...
import com.wx.model.Stu;

/**
 * A data access object (DAO) providing persistence and search support for Stu
 ...
 * 
 * @see com.wx.model.Stu
 * @author MyEclipse Persistence Tools
 */

public class StuDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory.getLogger(StuDAO.class);
	// property constants
	...
	public Stu findById(java.math.BigDecimal id) {
		log.debug("getting Stu instance with id: " + id);
		try {
			Stu instance = (Stu) getHibernateTemplate().get("com.wx.model.Stu",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Stu instance) {
	...
	public Stu merge(Stu detachedInstance) {
		...
		try {
			Stu result = (Stu) getHibernateTemplate().merge(detachedInstance);
